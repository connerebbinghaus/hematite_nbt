...
impl Blob {
    ...
    pub fn from_reader<R>(src: &mut R, config: &Config) -> Result<Blob>
    where
        R: io::Read,
    {
        let (tag, title) = raw::emit_next_header(src, config)?;
        ...
        let content = Value::from_reader(tag, src, config)?;
        match content {
    ...
    pub fn from_gzip_reader<R>(src: &mut R, config: &Config) -> Result<Blob>
    where
        R: io::Read,
    {
        // Reads the gzip header, and fails if it is incorrect.
        let mut data = GzDecoder::new(src);
        Blob::from_reader(&mut data, config)
    }

    /// Extracts an `Blob` object from an `io::Read` source that is
    /// compressed using the zlib format.
    pub fn from_zlib_reader<R>(src: &mut R, config: &Config) -> Result<Blob>
    where
        R: io::Read,
    {
        Blob::from_reader(&mut ZlibDecoder::new(src), config)
    }
    ...
    {
        dst.write_u8(0x0a)?;
        raw::write_bare_string(&mut dst, &self.title, config)?;
        for (name, ref nbt) in self.content.iter() {
            dst.write_u8(nbt.id())?;
            raw::write_bare_string(&mut dst, name, config)?;
            nbt.to_writer(&mut dst, config)?;
        }
    ...
    pub fn to_gzip_writer<W>(&self, dst: &mut W, config: &Config) -> Result<()>
    where
        W: io::Write,
    {
        self.to_writer(&mut GzEncoder::new(dst, Compression::default()), config)
    }

    /// Writes the binary representation of this `Blob`, compressed using
    /// the Zlib format, to an `io::Write` dst.
    pub fn to_zlib_writer<W>(&self, dst: &mut W, config: &Config) -> Result<()>
    where
        W: io::Write,
    {
        self.to_writer(&mut ZlibEncoder::new(dst, Compression::default()), config)
    }
